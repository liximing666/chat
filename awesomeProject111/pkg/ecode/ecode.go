package ecode

import (
	"fmt"
	"github.com/pkg/errors"
	"strconv"
	"sync/atomic"
)

var (
	_messages atomic.Value         // NOTE: stored map[string]map[int]string
	_codes    = map[int]struct{}{} // register codes.
)

// Register register ecode message map.
func Register(cm map[int]string) {
	_messages.Store(cm)
}

//通用错误码添加方法
func add(e int) Code {
	if _, ok := _codes[e]; ok {
		panic(fmt.Sprintf("ecode: %d already exist", e))
	}
	_codes[e] = struct{}{}
	return Int(e)
}

//其他业务错误码添加方法
func New(e int) Code {
	if e <= 0 {
		panic("business ecode must greater than zero")
	}
	return add(e)
}

// Codes ecode error interface which has a code & message.
type Codes interface {
	// sometimes Error return Code in string form
	// NOTE: don't use Error in monitor report even it also work for now
	Error() string
	// Code get error code.
	Code() int
	// Message get code message.
	Message() string
	// Equal for compatible.
	// Deprecated: please use ecode.EqualError.
	Equal(error) bool
}

// A Code is an int error code spec.
type Code struct {
	Number int
	msg    string
}

func (e Code) Error() string {
	if len(e.msg) == 0 {
		return strconv.FormatInt(int64(e.Number), 10)
	}
	return e.msg
}

func (e Code) SetMessage(msg string) Code {
	ret := e
	ret.msg = msg
	return ret
}

// Code return error code
func (e Code) Code() int { return int(e.Number) }

// Message return error message
func (e Code) Message() string {
	if 0 < len(e.msg) {
		return e.msg
	}
	if cm, ok := _messages.Load().(map[int]string); ok {
		if msg, ok := cm[e.Code()]; ok {
			return msg
		}
	}
	return e.Error()
}

// Equal for compatible.
// Deprecated: please use ecode.EqualError.
func (e Code) Equal(err error) bool { return EqualError(e, err) }

// Int parse code int to error.
func Int(i int) Code { return Code{Number: i} }

// String parse code string to error.
func String(e string) Code {
	if e == "" {
		return OK
	}
	// try error string
	i, err := strconv.Atoi(e)
	if err != nil {
		return INTERNAL_ERR
	}
	return Code{Number: i}
}

// Cause cause from error to ecode.
func Cause(e error) Codes {
	if e == nil {
		return OK
	}
	ec, ok := errors.Cause(e).(Codes)
	if ok {
		return ec
	}
	return String(e.Error())
}

// Equal equal a and b by code int.
func Equal(a, b Codes) bool {
	if a == nil {
		a = OK
	}
	if b == nil {
		b = OK
	}
	return a.Code() == b.Code()
}

// EqualError equal error
func EqualError(code Codes, err error) bool {
	return Cause(err).Code() == code.Code()
}

func Init() {
	//注册错误码
	Register(AppendMap(CommonErrMsg, AssetCodeMsg))
}

func AppendMap(mapParams ...map[int]string) map[int]string {
	res := map[int]string{}
	for _, v := range mapParams {
		for k, m := range v {
			res[k] = m
		}
	}
	return res
}
